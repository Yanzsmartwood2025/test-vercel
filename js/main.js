...
if (document.getElementById('hero-slideshow')) {
    ...
    if (document.getElementById('product-grid')) {
        ...
        const createCategoryCard = (category) => {
            ...
            video.className = "w-full h-48 object-cover lazy-video"; // Added a class for selection
            video.loop = true;
            video.muted = true;
            video.playsInline = true;

            const source = document.createElement('source');
            source.setAttribute('data-src', category.video); // Use data-src
            source.type = "video/mp4";

            video.appendChild(source);
            videoContainer.appendChild(video);

            // Store the video element to be observed later
       ...
       }, { rootMargin: '0px 0px 50px 0px', threshold: 0.01 }); // Start loading when the video is 50px away from the viewport
